...
tok_backup(register struct tok_state *tok, register int c)
{
	if (c != EOF) {
		if (--tok->cur < tok->buf)
			...
			Py_FatalError("tok_backup: 已是暫存的開頭");
		if (*tok->cur != c)
			*tok->cur = c;
	}
}
...
indenterror(struct tok_state *tok)
{
	...
	if (tok->altwarning) {
		/*PySys_WriteStderr("%s: inconsistent use of tabs and spaces "
                                  "in indentation\n", tok->filename);*/
		PySys_WriteStderr("%s: 跳格和空格的混用前後不協調"
                                  ", 縮排錯誤\n", tok->filename);
		tok->altwarning = 0;
	}
	return 0;
		...
		char **p_end)
{
 ...
 again:
	...
	if (c == '#') {
		...
		static char encode[] = { BIG5, BIG5, BIG5, GBK, GBK, GBK, };
		...
		*tp = '\0';
		...
		for (i = 0; i < sizeof(encodingforms)/sizeof(encodingforms[0]); i++)
		{
			if ((tp = strstr(cbuf, encodingforms[i]))) {
				File_Encoding = encode[i];
				if (Py_VerboseFlag)
				    PySys_WriteStderr(
						/*"Current Chinese Encoding set to %s\n", *cp);*/
						"檔案的中文編碼設定為 %s\n", *cp);
			}
		}
	...
	if (isalpha(c) || c == '_' || ishighascii(c) ) {
		/* Process r"", u"" and ur"" */
		switch (c) {
		case 'r':
		case 'R':
