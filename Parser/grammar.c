...
adddfa(grammar *g, int type, char *name)
{
	...
	if (g->g_dfa == NULL)
		...
		Py_FatalError("不夠記憶體改變 dfa 的大小, 發生在 adddfa");
	d = &g->g_dfa[g->g_ndfas++];
	d->d_type = type;
	d->d_name = name;
	d->d_nstates = 0;
...
addstate(dfa *d)
{
	...
	if (d->d_state == NULL)
		...
		Py_FatalError("不夠記憶體改變 state 的大小, 發生在 addstate");
...
addarc(dfa *d, int from, int to, int lbl)
{
	...
	if (s->s_arc == NULL)
		...
		Py_FatalError("不夠記憶體改變 arc 表的大小, 發生在 addarc");
	a = &s->s_arc[s->s_narcs++];
	a->a_lbl = lbl;
	a->a_arrow = to;
}
...
addlabel(labellist *ll, int type, char *str)
{
	...
	if (ll->ll_label == NULL)
		...
		Py_FatalError("不夠記憶體改變 labellist 的大小, 發生在 addlabel");
