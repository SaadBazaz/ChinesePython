...
PyInt_AsLong(register PyObject *op)
{
	...
	if (op == NULL || (nb = op->ob_type->tp_as_number) == NULL ||
		...
		PyErr_SetString(PyExc_TypeError, "莱肚俱计");
		return -1;
	}
...
PyInt_FromString(char *s, char **pend, int base)
{
	...
	if ((base != 0 && base < 2) || base > 36) {
		...
		PyErr_SetString(PyExc_ValueError, "int()/俱计() 膀┏斗 >= 2  <= 36");
		return NULL;
	}
  ...
  bad:
		...
		sprintf(buffer, "int()/俱计() Μ岿粇把计: %.200s", s);
		PyErr_SetString(PyExc_ValueError, buffer);
		return NULL;
	}
	else if (errno != 0) {
		/*sprintf(buffer, "int() literal too large: %.200s", s);*/
		sprintf(buffer, "int/俱计() Μび:  %.200s", s);
		PyErr_SetString(PyExc_ValueError, buffer);
		return NULL;
	}
	if (pend)
...
int_lshift(PyIntObject *v, PyIntObject *w)
{
	...
	if (b < 0) {
		...
		PyErr_SetString(PyExc_ValueError, "璽簿");
		return NULL;
	}
	if (a == 0 || b == 0) {
		Py_INCREF(v);
