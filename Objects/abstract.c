...
PyObject_GetItem(PyObject *o, PyObject *key)
{
	...
	if (o->ob_type->tp_as_sequence) {
		...
		return type_error("序列的索引必須為整數");
	}

	/*return type_error("unsubscriptable object");*/
	return type_error("實體不接受索引項");
}
...
ternary_error:
	...
	PyErr_Format(PyExc_TypeError, "%s 不支援這個類型的運算元",
			op_name);
	return NULL;
}
...
PyNumber_Absolute(PyObject *o)
{
	...
	return type_error("abs()/絕對值() 收到不正確的運算元");
}
...
PySequence_Tuple(PyObject *v)
{
	...
	return type_error("tuple()/變元素組() 的參數必須是序列");
}
...
PySequence_List(PyObject *v)
{
	...
	return type_error("list()/變序列() 的參數必須是序列");
}
...
PySequence_Contains(PyObject *w, PyObject *v) /* v in w */
{
	...
	if (sq == NULL || sq->sq_item == NULL) {
		PyErr_SetString(PyExc_TypeError,
			...
			"'in' or 'not in'/'來算' 或 '不來算' 右面需要一個序列");
		return -1;
	}
...
PySequence_Index(PyObject *s, PyObject *o)
{
	...
	PyErr_SetString(PyExc_ValueError, "sequence.index(x): x 不存在於序列中");
	return -1;
}
