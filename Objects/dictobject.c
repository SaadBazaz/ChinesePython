...
PyDict_New(void)
{
	...
	if (dummy == NULL) { /* Auto-initialize dummy */
		dummy = PyString_FromStringAndEncode("<dummy key>",
					Source_Encoding);
		if (dummy == NULL)
			return NULL;
#ifdef SHOW_CONVERSION_COUNTS
...
PyDict_GetItem(PyObject *op, PyObject *key)
{
	...
	test =  (mp->ma_lookup)(mp, key, hash)->me_value;
	return test;
}
...
dict_repr(dictobject *mp)
{
	...
	v = PyString_FromStringAndEncode("{", Current_Encoding);
	sepa = PyString_FromStringAndEncode(", ", Current_Encoding);
	colon = PyString_FromStringAndEncode(": ", Current_Encoding);
	any = 0;
	...
	PyString_ConcatAndDel(&v, PyString_FromStringAndEncode("}", 
					Current_Encoding));
	Py_ReprLeave((PyObject*)mp);
	Py_XDECREF(sepa);
	Py_XDECREF(colon);
...
dict_has_key(register dictobject *mp, PyObject *args)
{
	...
	long hash;
	register long ok;
	if (!PyArg_ParseTuple(args, "O:has_key/有", &key))
		return NULL;
#ifdef CACHE_HASH
	if (!PyString_Check(key) ||
	    (hash = ((PyStringObject *) key)->ob_shash) == -1)
...
dict_get(register dictobject *mp, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "O|O:get/取值", &key, &failobj))
		return NULL;
	if (mp->ma_table == NULL)
		goto finally;
...
dict_setdefault(register dictobject *mp, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "O|O:setdefault/設值", &key, &failobj))
		return NULL;
	if (mp->ma_table == NULL)
		goto finally;
...
static PyMethodDef mapp_methods[] = {
	{"has_key",	(PyCFunction)dict_has_key,      METH_VARARGS,
	 has_key__doc__},
	{"有",	(PyCFunction)dict_has_key,      METH_VARARGS,
	 has_key__doc__},
	{"get",         (PyCFunction)dict_get,          METH_VARARGS,
	 get__doc__},
	{"取值",         (PyCFunction)dict_get,          METH_VARARGS,
	 get__doc__},
	{"setdefault",  (PyCFunction)dict_setdefault,   METH_VARARGS,
	 setdefault_doc__},
	{"設值",  (PyCFunction)dict_setdefault,   METH_VARARGS,
	 setdefault_doc__},
	{"popitem",	(PyCFunction)dict_popitem,	METH_OLDARGS,
	 popitem__doc__},
	{"拿出",	(PyCFunction)dict_popitem,	METH_OLDARGS,
	 popitem__doc__},
	{"keys",	(PyCFunction)dict_keys,		METH_OLDARGS,
	keys__doc__},
	{"索引",	(PyCFunction)dict_keys,		METH_OLDARGS,
	keys__doc__},
	{"items",	(PyCFunction)dict_items,	METH_OLDARGS,
	 items__doc__},
	{"內容",	(PyCFunction)dict_items,	METH_OLDARGS,
	 items__doc__},
	{"values",	(PyCFunction)dict_values,	METH_OLDARGS,
	 values__doc__},
	{"值",	(PyCFunction)dict_values,	METH_OLDARGS,
	 values__doc__},
	{"update",	(PyCFunction)dict_update,	METH_OLDARGS,
	 update__doc__},
	{"更新",	(PyCFunction)dict_update,	METH_OLDARGS,
	 update__doc__},
	{"clear",	(PyCFunction)dict_clear,	METH_OLDARGS,
	 clear__doc__},
	{"清除",	(PyCFunction)dict_clear,	METH_OLDARGS,
	 clear__doc__},
	{"copy",	(PyCFunction)dict_copy,		METH_OLDARGS,
	 copy__doc__},
	{"抄錄",	(PyCFunction)dict_copy,		METH_OLDARGS,
	 copy__doc__},
	{NULL,		NULL}	/* sentinel */
};
...
{
	PyObject *kv, *rv;
	kv = PyString_FromStringAndEncode(key, Source_Encoding);
	if (kv == NULL)
		return NULL;
