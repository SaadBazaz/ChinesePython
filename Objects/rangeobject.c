...
range_repr(rangeobject *r)
{
	...
	if (r->start == 0 && r->step == 1)
		...
		sprintf(buf1, "簡範圍(%ld)", r->start + r->len * r->step);

	else if (r->step == 1)
		/*sprintf(buf1, "xrange(%ld, %ld)",*/
		sprintf(buf1, "簡範圍(%ld, %ld)",
			r->start,
			r->start + r->len * r->step);

	else
		/*sprintf(buf1, "xrange(%ld, %ld, %ld)",*/
		sprintf(buf1, "簡範圍(%ld, %ld, %ld)",
			r->start,
			r->start + r->len * r->step,
			r->step);
	...
	return PyString_FromStringAndEncode(r->reps == 1 ? buf1 : buf2, 
					Source_Encoding);
}
...
range_concat(rangeobject *r, PyObject *obj)
{
	...
	PyErr_SetString(PyExc_TypeError, "簡範圍 實體不可相粘合");
	return NULL;
}
...
range_tolist(rangeobject *self, PyObject *args)
{
	...
	if (! PyArg_ParseTuple(args, ":tolist/變序列"))
		return NULL;
...
range_getattr(rangeobject *r, char *name)
{
	...
	static PyMethodDef range_methods[] = {
		...
		{"變序列",	(PyCFunction)range_tolist, METH_VARARGS,
                 "變序列() -> 序列\n"
                 "傳回一個含有相同元素的序列"},
		{NULL,		NULL}
	};
	static struct memberlist range_members[] = {
		...
		{"步進",  T_LONG, offsetof(rangeobject, step), RO},
		{"start", T_LONG, offsetof(rangeobject, start), RO},
		{"始", T_LONG, offsetof(rangeobject, start), RO},
		{"stop",  T_LONG, 0, RO},
		{"終",  T_LONG, 0, RO},
		{NULL, 0, 0, 0}
	};
