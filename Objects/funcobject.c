...
PyFunction_SetDefaults(PyObject *op, PyObject *defaults)
{
	...
	if (defaults == Py_None)
		...
		PyErr_SetString(PyExc_SystemError, "箇砞把计ぃ琌じ舱");
		return -1;
	}
...
PyFunction_SetClosure(PyObject *op, PyObject *closure)
{
	...
	if (closure == Py_None)
		...
		PyErr_SetString(PyExc_SystemError, "closure/超 ぃ琌じ舱");
		return -1;
	}
...
static struct memberlist func_memberlist[] = {
        ...
        {"ㄧ计_絏",     T_OBJECT,     OFF(func_code)},
        {"func_globals",  T_OBJECT,     OFF(func_globals),      READONLY},
        {"ㄧ计_ノ",  T_OBJECT,     OFF(func_globals),      READONLY},
        {"func_name",     T_OBJECT,     OFF(func_name),         READONLY},
        {"ㄧ计_嘿",     T_OBJECT,     OFF(func_name),         READONLY},
        {"__name__",      T_OBJECT,     OFF(func_name),         READONLY},
        {"__嘿__",      T_OBJECT,     OFF(func_name),         READONLY},
        {"func_closure",  T_OBJECT,     OFF(func_closure),      READONLY},
        {"ㄧ计_超",  T_OBJECT,     OFF(func_closure),      READONLY},
        {"func_defaults", T_OBJECT,     OFF(func_defaults)},
        {"ㄧ计_箇砞", T_OBJECT,     OFF(func_defaults)},
        {"func_doc",      T_OBJECT,     OFF(func_doc)},
        {"ㄧ计_弧",      T_OBJECT,     OFF(func_doc)},
        {"__doc__",       T_OBJECT,     OFF(func_doc)},
        {"__弧__",       T_OBJECT,     OFF(func_doc)},
        {"func_dict",     T_OBJECT,     OFF(func_dict)},
        {"ㄧ计_ㄥ",     T_OBJECT,     OFF(func_dict)},
        {"__dict__",      T_OBJECT,     OFF(func_dict)},
        {"__ㄥ__",      T_OBJECT,     OFF(func_dict)},
        {NULL}  /* Sentinel */
};
	...
	if (strcmp(sname, "func_code") == 0 ||
		...
		if (value == NULL || !PyCode_Check(value)) {
			PyErr_SetString(
				...
				"func_code/ㄧ计_絏 斗砞 絏 龟砰");
			return -1;
		}
	}
	else if (strcmp(sname, "func_defaults") == 0 ||
		 strcmp(sname, "ㄧ计_箇砞") == 0 ) {
		...
		if (value != NULL && !PyTuple_Check(value)) {
			PyErr_SetString(
				...
				"func_defaults/ㄧ计_箇砞 斗砞じ舱");
			return -1;
		}
	}
	else if (!strcmp(sname, "func_dict") || !strcmp(sname, "__dict__") ||
		 !strcmp(sname, "ㄧ计_ㄥ") || !strcmp(sname, "__ㄥ__") ) {
...
func_repr(PyFunctionObject *op)
{
	...
	if (op->func_name == Py_None)
		...
		sprintf(buf, "<拔ㄧ计 %p>", op);
	else
		/*sprintf(buf, "<function %.100s at %p>",*/
		sprintf(buf, "<ㄧ计 %.100s  %p>",
