...
inc_count(PyTypeObject *tp)
{
	if (tp->tp_alloc == 0) {
		...
		if (tp->tp_next != NULL) /* sanity check */
			...
			Py_FatalError("XXX inc_count 合法檢查");
...
PyObject_Print(PyObject *op, FILE *fp, int flags)
{
	...
	if (PyOS_CheckStack()) {
		...
		PyErr_SetString(PyExc_MemoryError, "堆壘溢值");
		return -1;
	}
#endif
...
PyObject_Repr(PyObject *v)
{
	...
	if (v == NULL)
		...
		return PyString_FromStringAndEncode("<無>", Source_Encoding);
	else if (v->ob_type->tp_repr == NULL) {
		char buf[120];
		/*sprintf(buf, "<%.80s object at %p>",*/
		sprintf(buf, "<%.80s 實體於 %p>",
			v->ob_type->tp_name, v);
		...
		if (!PyString_Check(res)) {
			PyErr_Format(PyExc_TypeError,
				     ...
				     "__repr__/__以文字表示__ 傳回的不是字串類 (類型為 %.200s)",
				     res->ob_type->tp_name);
			Py_DECREF(res);
			return NULL;
		}
...
*/
	...
	if (!PyString_Check(res)) {
		PyErr_Format(PyExc_TypeError,
			     ...
			     "__str__/__變字串__ 傳回的不是字串類 (類型 %.200s)",
			     res->ob_type->tp_name);
		Py_DECREF(res);
		return NULL;
	}
...
PyObject_Unicode(PyObject *v)
{
	...
	if (v == NULL)
		...
		res = PyString_FromStringAndEncode("<無>", Source_Encoding);
		...
		if (!PyInstance_Check(v) ||
			...
			if (!PyInstance_Check(v) ||
		    	    (func = PyObject_GetAttr(v, strstr_cp)) == NULL) {
				PyErr_Clear();
				res = PyObject_Repr(v);
			}
			else {
...
PyObject_Hash(PyObject *v)
{
	...
	PyErr_SetString(PyExc_TypeError, "此類型無法計算拼揍值");
	return -1;
}
...
PyNumber_Coerce(PyObject **pv, PyObject **pw)
{
	...
	PyErr_SetString(PyExc_TypeError, "數字無法轉化成同一類型");
	return -1;
}
...
PyCallable_Check(PyObject *x)
{
	...
	if (PyInstance_Check(x)) {
		...
		if (call == NULL)
			call = PyObject_GetAttrString(x, "__呼叫__");
		if (call == NULL) {
			PyErr_Clear();
			return 0;
		}
...
NotImplemented_repr(PyObject *op)
{
	...
	return PyString_FromStringAndEncode("尚未完工", Source_Encoding);
}
...
_Py_ForgetReference(register PyObject *op)
{
	...
	if (op->ob_refcnt < 0)
		...
		Py_FatalError("UNREF 一個負的參量");
	if (op == &refchain ||
	    op->_ob_prev->_ob_next != op || op->_ob_next->_ob_prev != op)
		/*Py_FatalError("UNREF invalid object");*/
		Py_FatalError("UNREF 一個無效的實體");
#ifdef SLOW_UNREF_CHECK
...
_PyTrash_deposit_object(PyObject *op)
{
	...
	if (PyTuple_Check(op))
		...
		Py_FatalError("此類型沒有 GC 支援 -- 內部程式寫錯");
