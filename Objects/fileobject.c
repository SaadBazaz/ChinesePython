...
PyFile_FromFile(FILE *fp, char *name, char *mode, int (*close)(FILE *))
{
	...
	if (f == NULL)
		return NULL;
	f->f_fp = NULL;
	f->f_name = PyString_FromStringAndEncode(name, Source_Encoding);
	f->f_mode = PyString_FromStringAndEncode(mode, Source_Encoding);
	f->f_close = close;
	f->f_softspace = 0;
...
PyFile_FromString(char *name, char *mode)
{
	...
	if (f->f_fp == NULL) {
		...
		if ( errno == 0 ) {
			...
			PyErr_SetString(PyExc_IOError, "無法打開文件");
			Py_DECREF(f);
			return NULL;
		}
#endif
...
err_closed(void)
{
	...
	PyErr_SetString(PyExc_ValueError, "對已關閉的文件進行存取");
	return NULL;
}
...
file_repr(PyFileObject *f)
{
	...
	sprintf(buf, "<%s 檔案 '%.256s', 模式 '%.10s' at %p>",
...
file_seek(PyFileObject *f, PyObject *args)
{
	...
	if (f->f_fp == NULL)
		return err_closed();
	whence = 0;
	if (!PyArg_ParseTuple(args, "O|i:seek/定位", &offobj, &whence))
		return NULL;
#if !defined(HAVE_LARGEFILE_SUPPORT)
...
file_truncate(PyFileObject *f, PyObject *args)
{
	...
	if (f->f_fp == NULL)
		return err_closed();
	newsizeobj = NULL;
	if (!PyArg_ParseTuple(args, "|O:truncate/切斷", &newsizeobj))
		return NULL;
	if (newsizeobj != NULL) {
#if !defined(HAVE_LARGEFILE_SUPPORT)
...
file_read(PyFileObject *f, PyObject *args)
{
	...
	if (f->f_fp == NULL)
		return err_closed();
	if (!PyArg_ParseTuple(args, "|l:read/讀入", &bytesrequested))
		return NULL;
	if (bytesrequested < 0)
...
#define INCBUFSIZE 1000
	...
	for (;;) {
		...
		if (p == NULL) {
			...
			if (PyErr_CheckSignals())
				return NULL;
			v = PyString_FromStringAndSizeAndEncode(buf, 
					pvfree - buf, Source_Encoding);
			return v;
		}
	...
	v = PyString_FromStringAndSizeAndEncode((char*)NULL, 
					(int)total_v_size, Source_Encoding);
	if (v == NULL)
		return v;
...
PyFile_GetLine(PyObject *f, int n)
{
	...
	if (PyFile_Check(f)) {
		...
		if (reader == NULL)
			reader = PyObject_GetAttrString(f, "讀一行");
		if (reader == NULL)
			return NULL;
		if (n <= 0)
			args = Py_BuildValue("()");
	...
	if (n < 0 && result != NULL && PyString_Check(result)) {
		...
		if (len == 0) {
			...
			if (result->ob_refcnt == 1)
				...
				PyObject *v;
				v = PyString_FromStringAndSizeAndEncode(s, 
						len-1, Source_Encoding);
				Py_DECREF(result);
				result = v;
			}
		}
	...
	/* change to current interpreter's encoding */
	if (result == NULL || 
		((PyStringObject *)result)->encode == Current_Encoding )
		return result;

	op = PyString_FromStringAndEncode(PyString_AS_STRING(result), 
						Current_Encoding);
	Py_DECREF(result);
	return op;
}
...
file_readline(PyFileObject *f, PyObject *args)
{
	...
	if (f->f_fp == NULL)
		return err_closed();
	if (!PyArg_ParseTuple(args, "|i:readline/讀一行", &n))
		return NULL;
	if (n == 0)
		return PyString_FromStringAndEncode("", Current_Encoding);
	if (n < 0)
		n = 0;
	return get_line(f, n);
}
...
file_xreadlines(PyFileObject *f, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, ":xreadlines/簡讀多行"))
		return NULL;
...
file_readlines(PyFileObject *f, PyObject *args)
{
	...
	if (f->f_fp == NULL)
		return err_closed();
	if (!PyArg_ParseTuple(args, "|l:readlines/讀多行", &sizehint))
		return NULL;
	...
	for (;;) {
		...
		if (p == NULL) {
			...
			if (big_buffer == NULL) {
				...
				big_buffer = 
					PyString_FromStringAndSizeAndEncode(
					NULL, buffersize, Current_Encoding);
				if (big_buffer == NULL)
					goto error;
		...
		do {
			...
			p++;
			line = PyString_FromStringAndSizeAndEncode(q, 
						p-q, Current_Encoding);
			if (line == NULL)
				goto error;
	...
	if (nfilled != 0) {
		/* Partial last line */
		line = PyString_FromStringAndSizeAndEncode(buffer, 
						nfilled, Current_Encoding);
		if (line == NULL)
			goto error;
		if (sizehint > 0) {
	...
	for (;;) {
		...
		for (i = 0; i < j; i++) {
			...
			if (!PyString_Check(v)) {
				...
				line = PyString_FromStringAndSizeAndEncode(
						buffer, len, Source_Encoding);
				if (line == NULL)
					goto error;
				Py_DECREF(v);
...
static struct memberlist file_memberlist[] = {
	...
	{"軟空格",	T_INT,		OFF(f_softspace)},
	{"mode",	T_OBJECT,	OFF(f_mode),	RO},
	{"模式",	T_OBJECT,	OFF(f_mode),	RO},
	{"name",	T_OBJECT,	OFF(f_name),	RO},
	{"名稱",	T_OBJECT,	OFF(f_name),	RO},
...
PyFile_SoftSpace(PyObject *f, int newflag)
{
	...
	if (f == NULL) {
		...
		if (v == NULL)
			v = PyObject_GetAttrString(f, "軟空格");
		if (v == NULL)
			PyErr_Clear();
		else {
			if (PyInt_Check(v))
		...
		if (v == NULL)
			...
			if (PyObject_SetAttrString(f, "softspace", v) != 0 ||
			    PyObject_SetAttrString(f, "軟空格", v) != 0 )
				PyErr_Clear();
			Py_DECREF(v);
		}
	}
...
PyFile_WriteObject(PyObject *v, PyObject *f, int flags)
{
	...
	if (f == NULL) {
		...
		PyErr_SetString(PyExc_TypeError, "對 NULL 文件進行輸出");
		return -1;
	}
	else if (PyFile_Check(f)) {
	...
	if (writer == NULL)
		writer = PyObject_GetAttrString(f, "輸出");
	if (writer == NULL)
		return -1;
	...
	if (f == NULL) {
		...
		if (!PyErr_Occurred())
			PyErr_SetString(PyExc_SystemError,
					...
					"PyFile_WriteString 的參數是 null 文件");
		return -1;
	}
	else if (PyFile_Check(f)) {
		FILE *fp = PyFile_AsFile(f);
		...
		PyObject *v = PyString_FromStringAndEncode(s, Source_Encoding);
		int err;
		if (v == NULL)
			return -1;
...
int PyObject_AsFileDescriptor(PyObject *o)
{
	...
	else if ((meth = PyObject_GetAttrString(o, "檔號")) != NULL)
	{
		PyObject *fno = PyEval_CallObject(meth, NULL);
		Py_DECREF(meth);
		if (fno == NULL)
