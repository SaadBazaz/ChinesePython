...
PyModule_New(char *name)
{
	...
	if (PyDict_SetItemString(m->md_dict, "__name__", nameobj) != 0)
		goto fail;
	if (PyDict_SetItemString(m->md_dict, "__名稱__", nameobj) != 0)
		goto fail;
	if (PyDict_SetItemString(m->md_dict, "__doc__", Py_None) != 0)
		goto fail;
	if (PyDict_SetItemString(m->md_dict, "__說明__", Py_None) != 0)
		goto fail;
	Py_DECREF(nameobj);
	return (PyObject *)m;
...
PyModule_GetName(PyObject *m)
{
	...
	if (nameobj == NULL || !PyString_Check(nameobj)) {
		...
		PyErr_SetString(PyExc_SystemError, "模組沒有名稱");
		return NULL;
	    }
	}
...
PyModule_GetFilename(PyObject *m)
{
	...
	if (fileobj == NULL || !PyString_Check(fileobj)) {
		...
		if (fileobj == NULL || !PyString_Check(fileobj)) {
		   ...
		   PyErr_SetString(PyExc_SystemError, "模組沒有檔案名");
		   return NULL;
		}
	}
		...
		if (value != Py_None && PyString_Check(key)) {
			...
			if (s[0] == '_' && s[1] != '_') {
				if (Py_VerboseFlag > 1)
				    ...
				    PySys_WriteStderr("#   清除/clear[1] %s\n", s);
		...
		if (value != Py_None && PyString_Check(key)) {
			...
			if (s[0] != '_' || strcmp(s, "__builtins__") != 0) {
				if (Py_VerboseFlag > 1)
				    ...
				    PySys_WriteStderr("#   清除/clear[2] %s\n", s);
				PyDict_SetItem(d, key, Py_None);
			}
		}
	}
...
module_repr(PyModuleObject *m)
{
	...
	if (filename == NULL) {
		...
		sprintf(buf, "<模組 '%.80s' (內建)>", name);
	} else {
		/*sprintf(buf, "<module '%.80s' from '%.255s'>", name, filename);*/
		sprintf(buf, "<模組 '%.80s' 來自 '%.255s'>", name, filename);
	}
