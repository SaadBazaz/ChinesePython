...
PyList_GetItem(PyObject *op, int i)
{
	...
	if (i < 0 || i >= ((PyListObject *)op) -> ob_size) {
		if (indexerr == NULL)
			indexerr = PyString_FromStringAndEncode(
				/*"list index out of range");*/
				"序列索引超出範圍", Source_Encoding);
...
list_repr(PyListObject *v)
{
	...
	if (i != 0) {
		if (i > 0)
			return PyString_FromStringAndEncode("[...]",
					Current_Encoding);
		return NULL;
	}
	s = PyString_FromStringAndEncode("[", Current_Encoding);
	comma = PyString_FromStringAndEncode(", ", Current_Encoding);
	...
	Py_XDECREF(comma);
	PyString_ConcatAndDel(&s, PyString_FromStringAndEncode("]", 
					Current_Encoding));
...
listinsert(PyListObject *self, PyObject *args)
{
	int i;
	PyObject *v;
	if (!PyArg_ParseTuple(args, "iO:insert/插入", &i, &v))
		return NULL;
	return ins(self, i, v);
}
...
{
	PyObject *v;
	if (!PyArg_ParseTuple_Compat1(args, "O:append/附加", &v))
		return NULL;
...
list_inplace_concat(PyListObject *self, PyObject *other)
{
	...
	other = PySequence_Fast(other, "+= 的右面須是一個序列");
	if (!other)
		return NULL;
...
listextend(PyListObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "O:extend/加長", &b))
		return NULL;

	/*b = PySequence_Fast(b, "list.extend() argument must be a sequence");*/
	b = PySequence_Fast(b, "序列.加長() 的參數須是序列型態");
	if (!b)
		return NULL;
...
listpop(PyListObject *self, PyObject *args)
{
	int i = -1;
	PyObject *v;
	if (!PyArg_ParseTuple(args, "|i:pop/拿出", &i))
		return NULL;
	if (self->ob_size == 0) {
		/* Special-case most common failure cause */
		/*PyErr_SetString(PyExc_IndexError, "pop from empty list");*/
		PyErr_SetString(PyExc_IndexError, "要在空序列中拿出值");
		return NULL;
	}
	if (i < 0)
		i += self->ob_size;
	if (i < 0 || i >= self->ob_size) {
		...
		PyErr_SetString(PyExc_IndexError, "拿出() 的索引值超出範圍");
		return NULL;
	}
	v = self->ob_item[i];
	Py_INCREF(v);
...
listsort(PyListObject *self, PyObject *args)
{
	...
	if (args != NULL) {
		if (!PyArg_ParseTuple(args, "|O:sort/排序", &compare))
			return NULL;
	}
...
{
	if (!PyArg_ParseTuple(args, ":reverse/反向"))
		return NULL;
	_listreverse(self);
	Py_INCREF(Py_None);
	return Py_None;
...
listindex(PyListObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple_Compat1(args, "O:index/找", &v))
		return NULL;
	...
	PyErr_SetString(PyExc_ValueError, "序列.找(某): 某 並不在序列中");
	return NULL;
}
...
listcount(PyListObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple_Compat1(args, "O:count/次數", &v))
		return NULL;
	for (i = 0; i < self->ob_size; i++) {
		int cmp = PyObject_RichCompareBool(self->ob_item[i], v, Py_EQ);
		if (cmp > 0)
...
listremove(PyListObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple_Compat1(args, "O:remove/移除", &v))
		return NULL;
	for (i = 0; i < self->ob_size; i++) {
		int cmp = PyObject_RichCompareBool(self->ob_item[i], v, Py_EQ);
		if (cmp > 0) {
	...
	PyErr_SetString(PyExc_ValueError, "序列.移除(某): 某 並不在序列中");
	return NULL;
}
...
static PyMethodDef list_methods[] = {
	...
	{"附加",	(PyCFunction)listappend,  METH_VARARGS, append_doc},
	{"insert",	(PyCFunction)listinsert,  METH_VARARGS, insert_doc},
	{"插入",	(PyCFunction)listinsert,  METH_VARARGS, insert_doc},
	{"extend",      (PyCFunction)listextend,  METH_VARARGS, extend_doc},
	{"加長",      (PyCFunction)listextend,  METH_VARARGS, extend_doc},
	{"pop",		(PyCFunction)listpop, 	  METH_VARARGS, pop_doc},
	{"拿出",		(PyCFunction)listpop, 	  METH_VARARGS, pop_doc},
	{"remove",	(PyCFunction)listremove,  METH_VARARGS, remove_doc},
	{"移除",	(PyCFunction)listremove,  METH_VARARGS, remove_doc},
	{"index",	(PyCFunction)listindex,   METH_VARARGS, index_doc},
	{"找",	(PyCFunction)listindex,   METH_VARARGS, index_doc},
	{"count",	(PyCFunction)listcount,   METH_VARARGS, count_doc},
	{"次數",	(PyCFunction)listcount,   METH_VARARGS, count_doc},
	{"reverse",	(PyCFunction)listreverse, METH_VARARGS, reverse_doc},
	{"反向",	(PyCFunction)listreverse, METH_VARARGS, reverse_doc},
	{"sort",	(PyCFunction)listsort, 	  METH_VARARGS, sort_doc},
	{"排序",	(PyCFunction)listsort, 	  METH_VARARGS, sort_doc},
	{NULL,		NULL}		/* sentinel */
};
