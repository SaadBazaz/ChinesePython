...
PyBuffer_FromObject(PyObject *base, int offset, int size)
{
	     ...
	     pb->bf_getsegcount == NULL )
	{
		...
		PyErr_SetString(PyExc_TypeError, "應為一暫存對象");
		return NULL;
	}
...
buffer_repr(PyBufferObject *self)
{
	...
	char *status = self->b_readonly ? "唯讀" : "讀寫";

	if ( self->b_base == NULL )
	{
		sprintf(buf, "<%s 暫存 指針 %p, 大小 %d 位於 %p>",
			status,
			self->b_ptr,
			self->b_size,
			self);
	...
	{
		sprintf(buf, "<%s 暫存在 %p, 指 %p, 大小 %d 位於 %p>",
			status,
			self->b_base,
			self->b_ptr,
			self->b_size,
...
buffer_hash(PyBufferObject *self)
{
	...
	if ( !self->b_readonly )
	{
		...
		PyErr_SetString(PyExc_TypeError, "此類型無法建立拼揍值");
		return -1;
	}
...
buffer_concat(PyBufferObject *self, PyObject *other)
{
	...
	ob = PyString_FromStringAndSizeAndEncode(NULL, 
		self->b_size + count, Source_Encoding);
...
buffer_item(PyBufferObject *self, int idx)
{
	if ( idx < 0 || idx >= self->b_size )
	{
		...
		PyErr_SetString(PyExc_IndexError, "暫存索引超出範圍");
		return NULL;
	}
	return PyString_FromStringAndSizeAndEncode(
		(char *)self->b_ptr + idx, 1, Source_Encoding);
}
...
buffer_slice(PyBufferObject *self, int left, int right)
{
	...
	if ( right < left )
		right = left;
	return PyString_FromStringAndSizeAndEncode((char *)self->b_ptr + left,
					  right - left, Source_Encoding);
}
...
buffer_getwritebuf(PyBufferObject *self, int idx, void **pp)
{
	if ( self->b_readonly )
	{
		...
		PyErr_SetString(PyExc_TypeError, "這個暫存是唯讀的");
		return -1;
	}
