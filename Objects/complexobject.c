...
complex_div(PyComplexObject *v, PyComplexObject *w)
{
	...
	if (errno == EDOM) {
		...
		PyErr_SetString(PyExc_ZeroDivisionError, "複數除法");
		return NULL;
	}
...
complex_remainder(PyComplexObject *v, PyComplexObject *w)
{
	...
	if (errno == EDOM) {
		...
		PyErr_SetString(PyExc_ZeroDivisionError, "複數餘數");
		return NULL;
	}
...
complex_divmod(PyComplexObject *v, PyComplexObject *w)
{
	...
	if (errno == EDOM) {
		...
		PyErr_SetString(PyExc_ZeroDivisionError, "複數 divmod/商餘數()");
		return NULL;
	}
	div.real = floor(div.real); /* Use the floor of the real part. */
	div.imag = 0.0;
...
complex_pow(PyComplexObject *v, PyObject *w, PyComplexObject *z)
{
	...
	long int_exponent;
 	...
 	if ((PyObject *)z!=Py_None) {
		...
		PyErr_SetString(PyExc_ValueError, "複數冪次");
		return NULL;
	}
...
{
	Py_complex c;
	if (!PyArg_ParseTuple(args, ":conjugate/共軛"))
		return NULL;
...
{
	if (strcmp(name, "real") == 0 || strcmp(name, "實部")==0)
		return (PyObject *)PyFloat_FromDouble(self->cval.real);
	else if (strcmp(name, "imag") == 0 || strcmp(name,"虛部")==0)
		return (PyObject *)PyFloat_FromDouble(self->cval.imag);
	else if (strcmp(name, "__members__") == 0 || strcmp(name,"__成員__")==0)
		return Py_BuildValue("[ssss]", "imag", "real", "虛部", "實部");
