...
PyFloat_FromString(PyObject *v, char **pend)
{
	...
	if (end == s) {
		...
		sprintf(buffer, "float()/疊翴计() 把计ぃ癸: %.200s", s);
...
float_div(PyObject *v, PyObject *w)
{
	...
	if (b == 0.0) {
		...
		PyErr_SetString(PyExc_ZeroDivisionError, "疊翴计埃箂");
		return NULL;
	}
...
float_rem(PyObject *v, PyObject *w)
{
	...
	if (wx == 0.0) {
		...
		PyErr_SetString(PyExc_ZeroDivisionError, "疊翴计埃计箂");
		return NULL;
	}
...
float_divmod(PyObject *v, PyObject *w)
{
	...
	if (wx == 0.0) {
		...
		PyErr_SetString(PyExc_ZeroDivisionError, "坝緇计()/divmod() 埃计箂");
		return NULL;
	}
