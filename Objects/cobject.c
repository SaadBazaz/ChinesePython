                             ...
                             void (*destr)(void *, void *))
{
    ...
    if (!desc) {
        PyErr_SetString(PyExc_TypeError,
                        ...
                        " description");*/
                        "PyCObject_FromVoidPtrAndDesc 調用時用了 null"
                        "作為描述參數");
        return NULL;
    }
    self = PyObject_NEW(PyCObject, &PyCObject_Type);
...
PyCObject_AsVoidPtr(PyObject *self)
{
    if (self) {
        ...
        PyErr_SetString(PyExc_TypeError,
                        ...
                        "PyCObject_AsVoidPtr 調用時傳入的不是 C-實體");
    }
    if (!PyErr_Occurred())
        PyErr_SetString(PyExc_TypeError,
                        /*"PyCObject_AsVoidPtr called with null pointer");*/
                        "PyCObject_AsVoidPtr 調用時傳入了 null 指針");
    return NULL;
}
...
PyCObject_GetDesc(PyObject *self)
{
    if (self) {
        ...
        PyErr_SetString(PyExc_TypeError,
                        ...
                        "PyCObject_GetDesc 調用時傳入非 C-實體");
    }
    if (!PyErr_Occurred())
        PyErr_SetString(PyExc_TypeError,
                        /*"PyCObject_GetDesc called with null pointer");*/
                        "PyCObject_GetDesc 調用時傳入 null 指針");
    return NULL;
}
