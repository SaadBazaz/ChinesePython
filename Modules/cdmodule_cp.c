...
static PyMethodDef cdplayer_methods[] = {
	{"容許刪除",	(PyCFunction)CD_allowremoval,	1},
	{"最佳讀取量",	(PyCFunction)CD_bestreadsize,	1},
	{"關上",		(PyCFunction)CD_close,		1},
	{"退出",		(PyCFunction)CD_eject,		1},
	{"讀取狀態",		(PyCFunction)CD_getstatus,		1},
	{"讀音軌資料",	(PyCFunction)CD_gettrackinfo,	1},
	{"msftoblock",		(PyCFunction)CD_msftoblock,		1},
	{"播放",		(PyCFunction)CD_play,		1},
	{"播放abs",		(PyCFunction)CD_playabs,		1},
	{"播放音軌",		(PyCFunction)CD_playtrack,		1},
	{"播放音軌abs",	(PyCFunction)CD_playtrackabs,	1},
	{"防止刪除",	(PyCFunction)CD_preventremoval,	1},
	{"讀da",		(PyCFunction)CD_readda,		1},
	{"找",		(PyCFunction)CD_seek,		1},
	{"找區塊",		(PyCFunction)CD_seekblock,		1},
	{"找音軌",		(PyCFunction)CD_seektrack,		1},
	{"停",		(PyCFunction)CD_stop,		1},
	{"暫停鍵",		(PyCFunction)CD_togglepause,   	1},
	{NULL,			NULL} 		/* sentinel */
};
...
initcd_cp(void)
{
	...
	/* m = Py_InitModule("唱碟", CD_methods); */
	m = Py_InitModule("cd_cp", CD_methods); 
	d = PyModule_GetDict(m);

	CdError = PyErr_NewException("唱盤.錯誤", NULL, NULL);
	PyDict_SetItemString(d, "錯誤", CdError);

	/* Identifiers for the different types of callbacks from the parser */
	PyDict_SetItemString(d, "音響", PyInt_FromLong((long) cd_audio));
	PyDict_SetItemString(d, "pnum", PyInt_FromLong((long) cd_pnum));
	PyDict_SetItemString(d, "索引", PyInt_FromLong((long) cd_index));
	PyDict_SetItemString(d, "ptime", PyInt_FromLong((long) cd_ptime));
	PyDict_SetItemString(d, "atime", PyInt_FromLong((long) cd_atime));
	PyDict_SetItemString(d, "目錄", PyInt_FromLong((long) cd_catalog));
	PyDict_SetItemString(d, "ident", PyInt_FromLong((long) cd_ident));
	PyDict_SetItemString(d, "控制", PyInt_FromLong((long) cd_control));

	/* Block size information for digital audio data */
	PyDict_SetItemString(d, "資料大小",
			   PyInt_FromLong((long) CDDA_DATASIZE));
	PyDict_SetItemString(d, "區塊大小",
			   PyInt_FromLong((long) CDDA_BLOCKSIZE));

	/* Possible states for the cd player */
	PyDict_SetItemString(d, "錯誤", PyInt_FromLong((long) CD_ERROR));
	PyDict_SetItemString(d, "沒有光碟", PyInt_FromLong((long) CD_NODISC));
	PyDict_SetItemString(d, "準備好了", PyInt_FromLong((long) CD_READY));
	PyDict_SetItemString(d, "播放中", PyInt_FromLong((long) CD_PLAYING));
	PyDict_SetItemString(d, "已暫停", PyInt_FromLong((long) CD_PAUSED));
	PyDict_SetItemString(d, "靜止", PyInt_FromLong((long) CD_STILL));
#ifdef CD_CDROM			/* only newer versions of the library */
	PyDict_SetItemString(d, "光盤", PyInt_FromLong((long) CD_CDROM));
#endif
}
