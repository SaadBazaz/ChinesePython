...
#else
#define PYTHONHOMEHELP "<prefix>/cpython2.1"
#endif

#include "pygetopt.h"

#define COPYRIGHT \
   "鍵入 \"版權\", \"鳴謝\", \"使用証\" 以取得更多資訊."
/*
   "鍵入 \"copyright\", \"credits\", \"license\" 以取得更多資訊."
*/
...
#define BASE_OPTS "c:bgdiOStuUvxXhVW:"
...
-b     : start interpreter in big5 encoding\n\
-d     : debug output from parser (also PYTHONDEBUG=x)\n\
-g     : start interpreter in gbk encoding\n\
...
Py_Main(int argc, char **argv)
{
	...
	if (Py_VerboseFlag ||
	   (command == NULL && filename == NULL && stdin_is_interactive)) {
		char *s, *r;
		char *h = "中蟒 0.4 (中文 Python) 基於原英文版 ";
		if(Current_Encoding == GBK) {
			s = big5_to_gbk(COPYRIGHT);
			r = big5_to_gbk(h);
     			fprintf(stderr, "%s%s on %s\n%s\n",
                        	r, Py_GetVersion(), Py_GetPlatform(), s);
			free(s);
			free(r);
		}
		else if(Current_Encoding == BIG5)
     			fprintf(stderr, "%s%s on %s\n%s\n", h, 
	...
	if (inspect && stdin_is_interactive &&
		...
		sts = PyRun_AnyFileFlags(stdin, "<標準輸出>", &cf) != 0;
