...
initcmath_cp(void)
{
	...
	/* m = Py_InitModule3("複數學", cmath_methods, module_doc); */
	m = Py_InitModule3("cmath_cp", cmath_methods, module_doc); 
	d = PyModule_GetDict(m);
	PyDict_SetItemString(d, "pi",
			     v = PyFloat_FromDouble(atan(1.0) * 4.0));
	Py_DECREF(v);
	PyDict_SetItemString(d, "圓周率",
			     v = PyFloat_FromDouble(atan(1.0) * 4.0));
	Py_DECREF(v);
	PyDict_SetItemString(d, "e", v = PyFloat_FromDouble(exp(1.0)));
	Py_DECREF(v);
	PyDict_SetItemString(d, "自然對數底", v = PyFloat_FromDouble(exp(1.0)));
	Py_DECREF(v);
}
