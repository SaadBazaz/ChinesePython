...
static struct PyMethodDef reg_methods[] = {
	{"比對",	(PyCFunction)regobj_match, 1},
	{"找尋",	(PyCFunction)regobj_search, 1},
	{"分組",	(PyCFunction)regobj_group, 1},
	{NULL,		NULL}		/* sentinel */
};
...
static struct PyMethodDef regex_global_methods[] = {
	...
	{"比對",	regex_match, 0},
	{"找尋",	regex_search, 0},
	{"設定句法",	regex_set_syntax, 0},
	{"取得句法",  regex_get_syntax, 0},
	{NULL,		NULL}		     /* sentinel */
};
...
initregex_cp(void)
{
	...
	/* m = Py_InitModule("正規式", regex_global_methods); */
	m = Py_InitModule("regex_cp", regex_global_methods); 
	d = PyModule_GetDict(m);
	
	/* Initialize regex.error exception */
	v = RegexError = PyErr_NewException("正規式.錯誤", NULL, NULL);
	if (v == NULL || PyDict_SetItemString(d, "錯誤", v) != 0)
		goto finally;
	...
	if (PyDict_SetItemString(d, "大小寫不分", v) < 0)
		goto finally;
	Py_DECREF(v);
