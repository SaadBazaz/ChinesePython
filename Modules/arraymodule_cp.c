...
{
	char x;
	if (!PyArg_Parse(v, "c;陣列中元素必須是字元態", &x))
		return -1;
	if (i >= 0)
		     ((char *)ap->ob_item)[i] = x;
	return 0;
...
b_setitem(arrayobject *ap, int i, PyObject *v)
{
	...
	if (!PyArg_Parse(v, "h;陣列中元素必須為整數態", &x))
		return -1;
	else if (x < -128) {
...
BB_setitem(arrayobject *ap, int i, PyObject *v)
{
	...
	if (!PyArg_Parse(v, "b;陣列中元素必須為整數態", &x))
		return -1;
	if (i >= 0)
		     ((char *)ap->ob_item)[i] = x;
	return 0;
...
HH_setitem(arrayobject *ap, int i, PyObject *v)
{
	...
	if (!PyArg_Parse(v, "i;陣列中元素必須為整數態", &x))
		return -1;
	else if (x < 0) {
		PyErr_SetString(PyExc_OverflowError,
			"unsigned short 小於最小值 (0)");
		return -1;
	}
	else if (x > USHRT_MAX) {
		PyErr_SetString(PyExc_OverflowError,
			"unsigned short 大於最大值 (USHRT_MAX)");
		return -1;
	}
	if (i >= 0)
...
II_setitem(arrayobject *ap, int i, PyObject *v)
{
	...
	if (PyLong_Check(v)) {
		...
		long y;
		if (!PyArg_Parse(v, "l;陣列中元素必須為整數態", &y))
			return -1;
		if (y < 0) {
			PyErr_SetString(PyExc_OverflowError,
				"不帶正負號的整數小於最小值 (0)");
...
{
	long x;
	if (!PyArg_Parse(v, "l;陣列中元素必須為整數值", &x))
		return -1;
	if (i >= 0)
		     ((long *)ap->ob_item)[i] = x;
	return 0;
...
LL_setitem(arrayobject *ap, int i, PyObject *v)
{
	...
	if (PyLong_Check(v)) {
		...
		long y;
		if (!PyArg_Parse(v, "l;陣列中元素必須為整數值", &y))
			return -1;
		if (y < 0) {
			PyErr_SetString(PyExc_OverflowError,
...
{
	float x;
	if (!PyArg_Parse(v, "f;陣列中元素必須為浮點數", &x))
		return -1;
	if (i >= 0)
...
{
	double x;
	if (!PyArg_Parse(v, "d;陣列中元素必須為浮點數", &x))
		return -1;
	if (i >= 0)
...
getarrayitem(PyObject *op, int i)
{
	...
	ap = (arrayobject *)op;
	if (i < 0 || i >= ap->ob_size) {
		PyErr_SetString(PyExc_IndexError, "陣列索引超出範圍");
		return NULL;
	}
...
array_count(arrayobject *self, PyObject *args)
{
	...
	PyObject *v;
        ...
        if (!PyArg_ParseTuple(args, "O:數目", &v))
		return NULL;
...
array_index(arrayobject *self, PyObject *args)
{
	...
	PyErr_SetString(PyExc_ValueError, "陣列.索引(x): x 並不在其中");
	return NULL;
}
...
PyMethodDef array_methods[] = {
	{"附加",	(PyCFunction)array_append, METH_VARARGS, append_doc},
	{"暫存區設置", (PyCFunction)array_buffer_info, METH_VARARGS, buffer_info_doc},
	{"位元互換",	(PyCFunction)array_byteswap, METH_VARARGS, byteswap_doc},
	{"數目",	(PyCFunction)array_count, METH_VARARGS, count_doc},
	{"廷伸",      (PyCFunction)array_extend, METH_VARARGS, extend_doc},
	{"自檔案",	(PyCFunction)array_fromfile, METH_VARARGS, fromfile_doc},
	{"自序列",	(PyCFunction)array_fromlist, METH_VARARGS, fromlist_doc},
	{"自字串",	(PyCFunction)array_fromstring, METH_VARARGS, fromstring_doc},
	{"索引",	(PyCFunction)array_index, METH_VARARGS, index_doc},
	{"插入",	(PyCFunction)array_insert, METH_VARARGS, insert_doc},
	{"彈出",		(PyCFunction)array_pop, METH_VARARGS, pop_doc},
	{"讀取",	(PyCFunction)array_fromfile, METH_VARARGS, fromfile_doc},
	{"移除",	(PyCFunction)array_remove, METH_VARARGS, remove_doc},
	{"反向",	(PyCFunction)array_reverse, METH_VARARGS, reverse_doc},
/*	{"排序",	(PyCFunction)array_sort, METH_VARARGS, sort_doc},*/
	{"到檔案",	(PyCFunction)array_tofile, METH_VARARGS, tofile_doc},
	{"到序列",	(PyCFunction)array_tolist, METH_VARARGS, tolist_doc},
	{"到字串",	(PyCFunction)array_tostring, METH_VARARGS, tostring_doc},
	{"輸出",	(PyCFunction)array_tofile, METH_VARARGS, tofile_doc},
	{NULL,		NULL}		/* sentinel */
};
	...
	if (strcmp(name, "__members__") == 0) {
		...
		if (list) {
			PyList_SetItem(list, 0,
				       PyString_FromString("型態"));
			PyList_SetItem(list, 1,
				       PyString_FromString("元素大小"));
			if (PyErr_Occurred()) {
				Py_DECREF(list);
				list = NULL;
			}
...
"新陣列(型態碼 [, 初始值]) -> 陣列\n\
\n\
傳回一個新的陣列. 該陣列中的元素類型依型態碼而定. 如果給出初始值 \n\
則陣列會按該值初始化. (合法的初始值是序列或是字串)";

static PyMethodDef a_methods[] = {
	{"新陣列",	a_array, METH_VARARGS, a_array_doc},
	{NULL,		NULL}		/* sentinel */
};
...
initarray_cp(void)
{
	...
	/* m = Py_InitModule3("陣列", a_methods, module_doc); */
	m = Py_InitModule3("array_cp", a_methods, module_doc);

	d = PyModule_GetDict(m);
	PyDict_SetItemString(d, "ArrayType", (PyObject *)&Arraytype);
	PyDict_SetItemString(d, "陣列類", (PyObject *)&Arraytype);
