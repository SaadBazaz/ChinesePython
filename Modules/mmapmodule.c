/*
 ...
 /  $Id: mmapmodule.c,v 1.3 2002/05/19 07:42:48 glace Exp $
		     ...
		     PyObject *args)
{
	...
	if (!eol)
		...
		++eol;		/* we're interested in the position after the
				   newline. */
	result = PyString_FromStringAndSizeAndEncode(start, 
				(eol - start), Current_Encoding);
	self->pos += (eol - start);
	return (result);
}
...
mmap_item(mmap_object *self, int i)
{
	...
	return PyString_FromStringAndSizeAndEncode(self->data + i, 
					1, Current_Encoding);
}
...
mmap_slice(mmap_object *self, int ilow, int ihigh)
{
	...
	return PyString_FromStringAndSizeAndEncode(self->data + ilow, 
				ihigh-ilow, Current_Encoding);
}
