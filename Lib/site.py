...
if os.name == "posix" and os.path.basename(sys.path[-1]) == "Modules":
    ...
    sys.path.append(s)
    sys.path.append('/home/glace/chinesepython2.1.3-0.4/Lib/lib_cp')
    del get_platform, s
...
class _Printer:
    ...
    def __repr__(self):
        ...
        if len(self.__lines) <= self.MAXLINES:
            ...
            傳回 '''鍵入 %s() 顯示更多%s的信息.''' % ((self.__name,)*2)
...
__builtin__.版權 = _Printer('''版權''', '''2002 中蟒由中蟒開發小組製作\n\n'''
				+ sys.copyright)
...
__builtin__.鳴謝 = _Printer('''鳴謝''', '''\
中蟒是基於 Python 作的中文化編程語言. 請到 www.chinesepython.org 瀏覽有關詳情.
Python 的發展有賴 CWI, CNRI, BeOpen.com, Digital Creations 以及數以千計的人士支持. 在 www.python.org 放有更詳細的資料.\n''')
...
# 我沒有用 '證' 字因為目前國標到的大五的轉換中只能出 '証' 字. 異體字. 唉, 頭好痛.

__builtin__.使用証 = _Printer(
    '''使用証''', "http://www.pythonlabs.com/products/python2.0/license.html",
    ["LICENSE.txt", "LICENSE"],
    [os.path.join(here, os.pardir), here, os.curdir])
