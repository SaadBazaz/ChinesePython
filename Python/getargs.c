	     ...
	     char *msgbuf, int toplevel)
{
	...
	if (!PySequence_Check(arg) || PyString_Check(arg)) {
		...
		sprintf(msgbuf,
			...
			toplevel ? "應有 %d 個參數而不是 %s" :
				   "必須是 %d-項序列而不是 %s",
			n, arg == Py_None ? "無" : arg->ob_type->tp_name);
		return msgbuf;
	}
...
convertsimple(PyObject *arg, char **p_format, va_list *p_va, char *msgbuf)
{
	...
	if (msg != NULL) {
		...
		sprintf(msgbuf, "須為 %.50s, 不是 %.50s", msg,
			arg == Py_None ? "無" : arg->ob_type->tp_name);
		msg = msgbuf;
	}
	return msg;
}
