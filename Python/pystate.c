	...
	for (p = &interp_head; ; p = &(*p)->next) {
		if (*p == NULL)
			Py_FatalError(
				...
				"PyInterpreterState_Delete: 不合法的 interp");
		if (*p == interp)
			break;
	}
	if (interp->tstate_head != NULL)
		/*Py_FatalError("PyInterpreterState_Delete: remaining threads");*/
		Py_FatalError("PyInterpreterState_Delete: 尚有線程未移除");
	*p = interp->next;
	HEAD_UNLOCK();
	PyMem_DEL(interp);
}
...
tstate_delete_common(PyThreadState *tstate)
{
	...
	if (tstate == NULL)
		...
		Py_FatalError("PyThreadState_Delete: tstate 的值為 NULL");
	interp = tstate->interp;
	if (interp == NULL)
		/*Py_FatalError("PyThreadState_Delete: NULL interp");*/
		Py_FatalError("PyThreadState_Delete: interp 的值為 NULL");
	HEAD_LOCK();
	for (p = &interp->tstate_head; ; p = &(*p)->next) {
		if (*p == NULL)
			Py_FatalError(
				/*"PyThreadState_Delete: invalid tstate");*/
				"PyThreadState_Delete: 不合法的 tstate");
		if (*p == tstate)
			break;
	}
	*p = tstate->next;
...
PyThreadState_Delete(PyThreadState *tstate)
{
	if (tstate == _PyThreadState_Current)
		...
		Py_FatalError("PyThreadState_Delete: tstate 的值在當前使用中");
...
PyThreadState_DeleteCurrent()
{
	...
	if (tstate == NULL)
		Py_FatalError(
			...
			"PyThreadState_DeleteCurrent: 並沒有當前使用中的 tstate");
...
PyThreadState_Get(void)
{
	if (_PyThreadState_Current == NULL)
		...
		Py_FatalError("PyThreadState_Get: 找不到當前的線程");
...
PyThreadState_GetDict(void)
{
	if (_PyThreadState_Current == NULL)
		...
		Py_FatalError("PyThreadState_GetDict: 找不到當前線程");
