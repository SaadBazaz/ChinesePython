...
int Current_Encoding; /* Needed by parsetok.c  and stringobject.c */
int File_Encoding; /* Needed by parsetok.c */
/* the order must match those defined in chineseencode.h */
char * Chinese_EncodingNames[] = { "dummy", "大五", "國標", };
...
Py_Initialize(void)
{
	...
	if ((p = getenv("LANG")) && !strncmp(p,"zh_TW",5))
		Current_Encoding = Current_Encoding ? Current_Encoding : BIG5;
	if ((p = getenv("LANG")) && !strncmp(p,"zh_HK",5))
		Current_Encoding = Current_Encoding ? Current_Encoding : BIG5;
	if ((p = getenv("LANG")) && !strncmp(p,"zh_CN",5))
		Current_Encoding = Current_Encoding ? Current_Encoding : GBK;
	/* set the default if not set in anyway */
	Current_Encoding = Current_Encoding ? Current_Encoding : BIG5;
	...
	if (interp == NULL)
		...
		Py_FatalError("Py_Initialize: 無法產生首個即譯器");

	tstate = PyThreadState_New(interp);
	if (tstate == NULL)
		/*Py_FatalError("Py_Initialize: can't make first thread");*/
		Py_FatalError("Py_Initialize: 無法產生首個執行緒");
	(void) PyThreadState_Swap(tstate);

	interp->modules = PyDict_New();
	if (interp->modules == NULL)
		/*Py_FatalError("Py_Initialize: can't make modules dictionary");*/
		Py_FatalError("Py_Initialize: 無法產生模組字典");
	...
	if (bimod == NULL)
		...
		Py_FatalError("Py_Initialize: 無法初始 __內建__");
	...
	if (sysmod == NULL)
		...
		Py_FatalError("Py_Initialize: 無法初始 sys");
	...
	PyDict_SetItemString(interp->sysdict, "modules",
			     interp->modules);
	PyDict_SetItemString(interp->sysdict, "模組",
			     interp->modules);
...
Py_NewInterpreter(void)
{
	...
	if (!initialized)
		...
		Py_FatalError("Py_NewInterpreter: 應先調用 Py_Initialize ");
...
Py_EndInterpreter(PyThreadState *tstate)
{
	...
	if (tstate != PyThreadState_Get())
		...
		Py_FatalError("Py_EndInterpreter: 執行緒並不是最近的");
	if (tstate->frame != NULL)
		/*Py_FatalError("Py_EndInterpreter: thread still has a frame");*/
		Py_FatalError("Py_EndInterpreter: 執行緒仍擁有運行框");
	if (tstate != interp->tstate_head || tstate->next != NULL)
		/*Py_FatalError("Py_EndInterpreter: not the last thread");*/
		Py_FatalError("Py_EndInterpreter: 這不是最後的執行緒");
...
initmain(void)
{
	...
	if (m == NULL)
		...
		Py_FatalError("無法創建 __main__ 模組");
	d = PyModule_GetDict(m);
	if (PyDict_GetItemString(d, "__builtins__") == NULL) {
		PyObject *bimod = PyImport_ImportModule("__builtin__");
		if (bimod == NULL ||
		    PyDict_SetItemString(d, "__builtins__", bimod) != 0 ||
		    PyDict_SetItemString(d, "__內建__", bimod) != 0) 
			/*Py_FatalError("can't add __builtins__ to __main__");*/
			Py_FatalError("無法把 __內建__ 加至 __main__");
		Py_DECREF(bimod);
	}
	if (PyDict_SetItemString(d, "__名稱__", 
		 		PyString_FromStringAndEncode("__主程式__", 
				Source_Encoding)) != 0)
		Py_FatalError("無法把 __名稱__ 加至 __main__");
}
...
PyRun_InteractiveLoopFlags(FILE *fp, char *filename, PyCompilerFlags *flags)
{
	...
	if (v == NULL) {
		PySys_SetObject("ps1", 
			v = PyString_FromStringAndEncode(">>> ", Source_Encoding));
		PySys_SetObject("提示符1", 
			v = PyString_FromStringAndEncode(">>> ", Source_Encoding));
		Py_XDECREF(v);
	}
			...
			PyCompilerFlags *flags)
{
	...
	if (maybe_pyc_file(fp, filename, ext, closeit)) {
		...
		if( (fp = fopen(filename, "rb")) == NULL ) {
			fprintf(stderr, "python: cannot reopen .pyc file\n");
			/*fprintf(stderr, "中蟒: 無法重新打開 .pyc 檔案\n");*/
			return -1;
		}
...
PyErr_PrintEx(int set_sys_last_vars)
{
	...
	if (set_sys_last_vars) {
		...
		PySys_SetObject("最後類型", exception);
		PySys_SetObject("last_value", v);
		PySys_SetObject("最後值", v);
		PySys_SetObject("last_traceback", tb);
		PySys_SetObject("最後追蹤", tb);
	}
	...
	if (hook) {
		...
		if (result == NULL) {
			...
			PySys_WriteStderr("sys.excepthook 發生錯誤:\n");
		...
		PySys_WriteStderr("找不到 sys.excepthook \n");
		PyErr_Display(exception, v, tb);
	}
	...
	if (f == NULL)
		    ...
		    PyErr_GivenExceptionMatches(exception, PyExc_SyntaxError))
		{
			...
			if (!parse_syntax_error(v, &message, &filename,
				...
				char buf[10];
				PyFile_WriteString(" 來自 \"", f);
				if (filename == NULL)
					/*PyFile_WriteString("<string>", f);*/
					PyFile_WriteString("<字串>", f);
				else
					PyFile_WriteString(filename, f);
			        PyFile_WriteString("\", 第 ", f);
                                sprintf(buf, "%d 行的", lineno);
				PyFile_WriteString(buf, f);
				PyFile_WriteString("\n", f);
				if (text != NULL) {
					if (Current_Encoding == GBK) {
						char *s = gbk_to_big5(text);
						print_error_text(f, offset, s);
						PyMem_DEL(s);
					}
					else {
	     ...
	     PyCompilerFlags *flags)
{
	...
	if (v && flags) {
		...
		/*fprintf(stderr, "run_pyc_file: 巢狀範圍: %d\n",
			flags->cf_nested_scopes);*/
	}
	Py_DECREF(co);
	return v;
...
Py_FatalError(char *msg)
{
	...
	/*fprintf(stderr, "中蟒發生致命錯誤: %s\n", msg);*/
	sprintf(buf, "中蟒發生致命錯誤: %.256s\n",msg);
	if(Current_Encoding == GBK) 
		s = big5_to_gbk(buf);
	else
		s = buf;
...
call_sys_exitfunc(void)
{
	...
	if (exitfunc) {
		...
		PySys_SetObject("退出函數", (PyObject *)NULL);
		res = PyEval_CallObject(exitfunc, (PyObject *)NULL);
		if (res == NULL) {
			if (!PyErr_ExceptionMatches(PyExc_SystemExit)) {
				/*PySys_WriteStderr("Error in sys.exitfunc:\n");*/
				PySys_WriteStderr("sys.exitfunc 中發生異常:\n");
			}
			PyErr_Print();
		}
		Py_DECREF(exitfunc);
