...
unlock_import(void)
{
	...
	if (import_lock_thread != me)
		...
		Py_FatalError("unlock_import: 沒有鎖住");
	import_lock_level--;
...
PyImport_GetModuleDict(void)
{
	...
	if (interp->modules == NULL)
		...
		Py_FatalError("PyImport_GetModuleDict: 模組字典不存在!");
	...
	if (value != NULL && PyModule_Check(value)) {
		...
		for (p = sys_deletes; *p != NULL; p++) {
			if (Py_VerboseFlag)
				...
				PySys_WriteStderr("# 清理 sys.%s\n", *p);
			PyDict_SetItemString(dict, *p, Py_None);
		}
		for (p = sys_files; *p != NULL; p+=2) {
			if (Py_VerboseFlag)
				/*PySys_WriteStderr("# restore sys.%s\n", *p);*/
				PySys_WriteStderr("# 還原 sys.%s\n", *p);
...
_PyImport_FixupExtension(char *name, char *filename)
{
	...
	char *cp_name = name;
	static char *sys_name = "sys";
	static char *exceptions_name = "exceptions";
	/* -- ice begin
	printf("fixingExtension: %s %s\n",name, filename);
	 -- ice end */
	if (strcmp(name, "系統") == 0)
	   cp_name = sys_name;
	if (strcmp(name, "異常") == 0)
	   cp_name = exceptions_name;
	...
	mod = PyDict_GetItemString(modules, cp_name);
	if (mod == NULL || !PyModule_Check(mod)) {
		PyErr_Format(PyExc_SystemError,
		  /*"_PyImport_FixupExtension: module %.200s not loaded", name);*/
		  "_PyImport_FixupExtension: 模組 %.200s 無法載入", name);
		return NULL;
	}
...
_PyImport_FindExtension(char *name, char *filename)
{
	...
	if (Py_VerboseFlag)
		...
		PySys_WriteStderr("載入 %s # 早前已經載入的 (%s)\n",
			name, filename);
	return mod;
}
...
PyImport_ExecCodeModuleEx(char *name, PyObject *co, char *pathname)
{
	...
	if (PyDict_GetItemString(d, "__builtins__") == NULL) {
		if (PyDict_SetItemString(d, "__builtins__",
			...
			return NULL;
		if (PyDict_SetItemString(d, "__內建__",
					 PyEval_GetBuiltins()) != 0)
			return NULL;
	}
	...
	v = NULL;
	if (pathname != NULL) {
		v = PyString_FromStringAndEncode(pathname, Source_Encoding);
		if (v == NULL)
			PyErr_Clear();
	}
	if (v == NULL) {
	...
	if (PyDict_SetItemString(d, "__檔案__", v) != 0)
		PyErr_Clear(); /* Not important enough to report */
	Py_DECREF(v);
...
load_package(char *name, char *pathname)
{
	...
	if (Py_VerboseFlag)
		PySys_WriteStderr("import %s # directory %s\n",
			name, pathname);
	d = PyModule_GetDict(m);
	file = PyString_FromStringAndEncode(pathname, Source_Encoding);
	if (file == NULL)
		return NULL;
	...
	if (err == 0)
		err = PyDict_SetItemString(d, "__檔案__", file);
	if (err == 0)
		err = PyDict_SetItemString(d, "__路徑__", path);
	if (err != 0) {
		m = NULL;
		goto cleanup;
	}
	...
	if (fdp == NULL) {
		fdp = find_module("__初始__", path, buf, sizeof(buf), &fp);
		if (fdp == NULL) {
	    ...
	    FILE **p_fp)
{
	...
	printf("------ now get __builtins__\n");
	printf("------ it is:%s\n",
		PyStringAs_String(PyObject_Repr(builtins_dict)));
	 -- ice end */

	if (strlen(realname) > MAXPATHLEN) {
		/*PyErr_SetString(PyExc_OverflowError, "module name is too long");*/
		PyErr_SetString(PyExc_OverflowError, "模組名稱太長了");
		return NULL;
	}

	strcpy(name, realname);
	
	/* -- ice begin for source code level chinese module name
	if(ishighascii(name[0])) {
		if(!strcmp(realname,"陣列\0")) strcpy(name,"cp_array");
		}
	 -- ice end */
...
load_module(char *name, FILE *fp, char *buf, int type)
{
	...
	case C_EXTENSION:
		...
		/* ice add check
		printf("C_EXTENSION case:dynamic loading '%s - %s\n",name, buf);
		ice add end */
		break;
#endif
...
PyImport_ImportFrozenModule(char *name)
{
	...
	if (ispackage) {
		...
		err2 = PyDict_SetItemString(d, "__路徑__", s);
		Py_DECREF(s);
		if (err != 0 || err2 != 0)
			return err;
	}
...
PyImport_ImportModule(char *name)
{
	...
	pname = PyString_FromStringAndEncode(name, Source_Encoding);
	  ...
	  int *p_buflen)
{
	...
	if (strcmp(*p_name, "系統") == 0)
		name = sys_name;
	if (strcmp(*p_name, "異常") == 0)
		...
		int recursive)
{
	...
	for (i = 0; ; i++) {
		...
		if (PyString_AS_STRING(item)[0] == '*') {
			...
			if (all == NULL)
				all = PyObject_GetAttrString(mod, "__公開__");
			if (all == NULL)
					PyErr_Clear();
			else {
	...
	if ((m = PyDict_GetItemString(modules, fullname)) != NULL) {
		...
		if (fdp == NULL) {
			if (!PyErr_ExceptionMatches(PyExc_ImportError))
				return NULL;
			PyErr_Clear();
			altsubname = big5_to_gbk(subname);
			/*printf("%s\n", altsubname);*/
			fdp = find_module(altsubname, path, buf, MAXPATHLEN+1, &fp);
			PyMem_DEL(altsubname);
		}
		Py_XDECREF(path);
		if (fdp == NULL) {
			if (!PyErr_ExceptionMatches(PyExc_ImportError))
...
PyImport_ReloadModule(PyObject *m)
{
	...
	if (m != PyDict_GetItemString(modules, name)) {
		PyErr_Format(PyExc_ImportError,
			     ...
			     "reload/重載(): %.200s 模組未列於 sys.modules/系統.模組",
			     name);
		return NULL;
	}
	...
	if (subname == NULL)
		...
		parentname = PyString_FromStringAndSizeAndEncode(name, 
					(subname-name), Source_Encoding);
		if (parentname == NULL)
			return NULL;
		...
		if (parent == NULL) {
			PyErr_Format(PyExc_ImportError,
			    ...
			    "reload/重載(): %.200s 的父模組未列於 sys.modules/系統.模組",
			    name);
			return NULL;
		}
		subname++;
	...
	if (fdp == NULL)
		return NULL;
	/* ice begin
	printf("load module at 1900, this is relaod module\n");
	  ice end */
	m = load_module(name, fp, buf, fdp->type);
	if (fp)
		fclose(fp);
...
imp_get_magic(PyObject *self, PyObject *args)
{
	...
	return PyString_FromStringAndSizeAndEncode(buf, 4, Source_Encoding);
}
...
imp_load_dynamic(PyObject *self, PyObject *args)
{
	...
	/* ice begin
	printf("imp_load_dynamic, loading '%s - %s\n",name, pathname);
	 ice end */
	return m;
}
...
imp_load_module(PyObject *self, PyObject *args)
{
	...
	/* ice begin
 	printf("load module at 2269, imp_load_module\n"); 
	ice end */
...
";*/
static char doc_find_module[] = "\
找模組(名字, [路徑]) -> (檔案, 檔名, (檔尾, 模式, 類型))\n\
找尋模組.  如沒有給出 '路徑' 或為 '無' 則表示找\n\
內建模組,然後是凍結的模組,再找 sys.path/系統.路徑\n\
模組的名字不能帶有 '.' 符號; 要找套件中的模組可以給出模組名字\n\
再加用該套件的 __path__/__路徑__ 作為 '路徑' 參數\
";
...
";*/
static char doc_load_module[] = "\
載入模組(名字, 檔案, 檔名, (檔尾, 模式, 類型)) -> 模組\n\
加載模組, 參數其實就是 '找模組()' 函數的傳回值.\n\
模組名字必須是完整的, 也就是如果是屬於某個套件的話則\
要包括該套件的名字.\
";
...
static char doc_get_suffixes[] = "\
取檔尾() -> [(檔尾, 模式, 類型), ...]\n\
傳回一個序列, 其中包含 (檔尾, 模式, 類型) 元素組類, \n\
這些元素組形容 '找模組()' 將要尋找的檔案規格.\
";
...
static char doc_new_module[] = "\
新模組(名字) -> 模組\n\
建立一個新的模組. 不要把它加進 系統.模組/sys.modules 中.\n\
如果模組是屬於套件中的話,模組的名字一定要是完整的套件名字.\
";

static PyMethodDef imp_methods[] = {
	{"find_module",		imp_find_module,	1, doc_find_module},
	{"找模組",		imp_find_module,	1, doc_find_module},
	{"get_magic",		imp_get_magic,		1, doc_get_magic},
	{"取暗號",		imp_get_magic,		1, doc_get_magic},
	{"get_suffixes",	imp_get_suffixes,	1, doc_get_suffixes},
	{"取檔尾",	imp_get_suffixes,	1, doc_get_suffixes},
	{"load_module",		imp_load_module,	1, doc_load_module},
	{"載入模組",		imp_load_module,	1, doc_load_module},
	{"new_module",		imp_new_module,		1, doc_new_module},
	{"新模組",		imp_new_module,		1, doc_new_module},
