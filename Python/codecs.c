...
PyObject *normalizestring(const char *string)
{
	...
	if (len > INT_MAX) {
		...
		PyErr_SetString(PyExc_OverflowError, "字串太長");
		return NULL;
	}
	
    v = PyString_FromStringAndSizeAndEncode(NULL, (int)len, Source_Encoding);
    if (v == NULL)
	return NULL;
		     ...
		     const char *errors)
{
	...
	v = PyString_FromStringAndEncode(errors, Source_Encoding);
	if (v == NULL) {
	    Py_DECREF(args);
	    return NULL;
	}
