...
"%.100s: exec/執行 或 'import/載入 *' 在巢狀範圍中引起名稱混亂"
/*"%.100s: exec or 'import *' makes names ambiguous in nested scope"*/

#define GLOBAL_AFTER_ASSIGN \
"名稱 '%.400s' 在 global/共用 宣告前被賦值"
/*"name '%.400s' is assigned to before global declaration"*/

#define GLOBAL_AFTER_USE \
"名稱 '%.400s' 在 global/共用 宣告前被調用"
/*"name '%.400s' is used prior to global declaration"*/

#define LOCAL_GLOBAL \
"名稱 '%.400s' 作為函數的參數但同時被宣告為 global/共用"
/*"name '%.400s' is a function paramter and declared global"*/

#define LATE_FUTURE \
"from __future__ 的載入方法只能出現在檔案的開頭"
...
static struct memberlist code_memberlist[] = {
	...
	{"碼_參數數目",	T_INT,		OFF(co_argcount),	READONLY},
	{"co_nlocals",	T_INT,		OFF(co_nlocals),	READONLY},
	{"碼_私有數目",	T_INT,		OFF(co_nlocals),	READONLY},
	{"co_stacksize",T_INT,		OFF(co_stacksize),	READONLY},
	{"碼_堆棧大小",T_INT,		OFF(co_stacksize),	READONLY},
	{"co_flags",	T_INT,		OFF(co_flags),		READONLY},
	{"碼_旗號",	T_INT,		OFF(co_flags),		READONLY},
	{"co_code",	T_OBJECT,	OFF(co_code),		READONLY},
	{"碼_碼",	T_OBJECT,	OFF(co_code),		READONLY},
	{"co_consts",	T_OBJECT,	OFF(co_consts),		READONLY},
	{"碼_常數",	T_OBJECT,	OFF(co_consts),		READONLY},
	{"co_names",	T_OBJECT,	OFF(co_names),		READONLY},
	{"碼_名字",	T_OBJECT,	OFF(co_names),		READONLY},
	{"co_varnames",	T_OBJECT,	OFF(co_varnames),	READONLY},
	{"碼_變數名",	T_OBJECT,	OFF(co_varnames),	READONLY},
	{"co_freevars",	T_OBJECT,	OFF(co_freevars),	READONLY},
	{"碼_自由變數名",	T_OBJECT,	OFF(co_freevars),	READONLY},
	{"co_cellvars",	T_OBJECT,	OFF(co_cellvars),	READONLY},
	{"碼_胞變數名",	T_OBJECT,	OFF(co_cellvars),	READONLY},
	{"co_filename",	T_OBJECT,	OFF(co_filename),	READONLY},
	{"碼_檔案",	T_OBJECT,	OFF(co_filename),	READONLY},
	{"co_name",	T_OBJECT,	OFF(co_name),		READONLY},
	{"碼_名稱",	T_OBJECT,	OFF(co_name),		READONLY},
	{"co_firstlineno", T_INT,	OFF(co_firstlineno),	READONLY},
	{"碼_首行號", T_INT,	OFF(co_firstlineno),	READONLY},
	{"co_lnotab",	T_OBJECT,	OFF(co_lnotab),		READONLY},
	{"碼_行號表",	T_OBJECT,	OFF(co_lnotab),		READONLY},
	{NULL}	/* Sentinel */
};
...
code_repr(PyCodeObject *co)
{
	...
	sprintf(buf, "<碼概念 %.100s 於 %p, 檔案 \"%.300s\", 第 %d 行>",
...
intern_strings(PyObject *tuple)
{
	...
	for (i = PyTuple_GET_SIZE(tuple); --i >= 0; ) {
		...
		if (v == NULL || !PyString_Check(v)) {
			...
			Py_FatalError("在 code slot 找到非字串型態資料");
...
com_error(struct compiling *c, PyObject *exc, char *msg)
{
	...
	v = PyString_FromStringAndEncode(msg, Source_Encoding);
	if (v == NULL)
...
block_pop(struct compiling *c, int type)
{
	...
	if (c->c_block[c->c_nblocks] != type && c->c_errors == 0) {
		...
		com_error(c, PyExc_SystemError, "錯誤的 block pop");
	}
}
...
com_init(struct compiling *c, char *filename)
{
	...
	if ((c->c_code = PyString_FromStringAndSizeAndEncode((char *)NULL,
						1000, Source_Encoding)) == NULL)
		goto fail;
	...
	if ((c->c_locals = PyDict_New()) == NULL)
		goto fail;
	if ((c->c_lnotab = PyString_FromStringAndSizeAndEncode((char *)NULL,
						1000, Source_Encoding)) == NULL)
		goto fail;
	c->c_globals = NULL;
	c->c_varnames = NULL;
...
parsestr(char *s)
{
	...
	if (len > INT_MAX) {
		...
		PyErr_SetString(PyExc_OverflowError, "需要解析的字串太長了");
		return NULL;
	}
	if (s[--len] != quote) {
		PyErr_BadInternalCall();
	...
	if (len >= 4 && s[0] == quote && s[1] == quote) {
		...
		/* if triple code, then we know they are already
		   converted to Source_Encoding(BIG5)
		encode = Source_Encoding; 
		   not so any more -- glace
		*/
	}
	...
	if (rawmode || strchr(s, '\\') == NULL) {
		return PyString_FromStringAndSizeAndEncode(s, 
						len, encode);
	}
...
com_apply_trailer(struct compiling *c, node *n)
{
	...
	default:
		com_error(c, PyExc_SystemError,
			  ...
			  "com_apply_trailer: trailer 的類型不詳");
	}
}
...
cmp_type(node *n)
{
	...
	if (NCH(n) == 1) {
		...
		case NAME:	if (strcmp(STR(n), "in") == 0) return IN;
				if (strcmp(STR(n), "來自") == 0) return IN;
				if (strcmp(STR(n), "is") == 0) return IS;
				if (strcmp(STR(n), "是") == 0) return IS;
				if (strcmp(STR(n), "不是") == 0) return IS_NOT;
				if (strcmp(STR(n), "不來自") == 0) return NOT_IN;
		}
	}
	else if (NCH(n) == 2) {
		switch (TYPE(CHILD(n, 0))) {
...
com_assign_trailer(struct compiling *c, node *n, int assigning, node *augn)
{
	...
	default:
		...
		com_error(c, PyExc_SystemError, "trailer 類型不詳");
	}
}
...
com_augassign(struct compiling *c, node *n)
{
	...
	default:
		...
		com_error(c, PyExc_SystemError, "com_augassign: 不合法的算符");
		return;
	}
...
com_return_stmt(struct compiling *c, node *n)
{
	...
	if (!c->c_infunction) {
		...
		com_error(c, PyExc_SyntaxError, "'return/傳回' 出現在函數之外");
	}
	if (NCH(n) < 2) {
		com_addoparg(c, LOAD_CONST, com_addconst(c, Py_None));
		com_push(c, 1);
...
com_from_import(struct compiling *c, node *n)
{
	...
	if (NCH(n) > 1) {
		...
		&& strcmp(STR(CHILD(n, 1)), "名") != 0) {
			/*com_error(c, PyExc_SyntaxError, "invalid syntax");*/
			com_error(c, PyExc_SyntaxError, "句法錯誤");
			return;
		}
...
com_import_stmt(struct compiling *c, node *n)
{
	...
	if (STR(CHILD(n, 0))[0] == 'f' || STR(CHILD(n, 0))[0] == cp_s[0]) {
		PyObject *tup;
		...
		if (TYPE(CHILD(n, 3)) == STAR) {
			...
			for (i = 3; i < NCH(n); i += 2) {
				PyTuple_SET_ITEM(tup, (i-3)/2, 
					PyString_FromStringAndEncode(STR(
						CHILD(CHILD(n, i), 0)), 
						Source_Encoding));
			}
		}
	...
	if (i-- > 0 && c->c_block[i] == SETUP_LOOP) {
		...
		if (j >= 0) {
			...
			for (; i > j; --i) {
			  ...
			  "'continue/繼續' 不能放在 'finally/之後' 句子裏");
			  		return;
			  	}
			}
		}
	 ...
	 PyCompilerFlags *flags)
{
	...
	if (sc.c_errors == 0) {
		...
		PyErr_SetString(PyExc_SystemError, "弄丟了句法錯誤的記錄");
	}
 exit:
	if (base == NULL) {
...
symtable_check_shadow(struct symtable *st, PyObject *name, int flags)
{
	...
	*/
	...
	sprintf(buf, "私有名稱 '%.100s' in '%.100s' shadows "
		"use of '%.100s' as global in nested scope '%.100s'",
...
symtable_import(struct symtable *st, node *n)
{
	...
	*/
	if (STR(CHILD(n, 0))[0] == 'f' ||
	    STR(CHILD(n, 0))[0] == from_cp[0]) {  /* from */
		node *dotname = CHILD(n, 1);
		if (strcmp(STR(CHILD(dotname, 0)), "__future__") == 0 ||
		    strcmp(STR(CHILD(dotname, 0)), "__未來__") == 0) {
