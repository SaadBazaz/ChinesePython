...
w_object(PyObject *v, WFILE *p)
{
	...
	if (p->depth > MAX_MARSHAL_STACK_DEPTH) {
		...
		encode = ((PyStringObject *)v)->encode;
		w_byte(encode, p);
		w_long((long)n, p);
...
r_object(RFILE *p)
{
	...
	case TYPE_STRING:
		...
		if (n < 0) {
			...
			PyErr_SetString(PyExc_ValueError, "要儲存的資料有問題");
			return NULL;
		}
		v = PyString_FromStringAndSizeAndEncode((char *)NULL, n, 
						encode);
		if (v != NULL) {
...
PyMarshal_WriteObjectToString(PyObject *x) /* wrs_object() */
{
	WFILE wf;
	wf.fp = NULL;
	wf.str = PyString_FromStringAndSizeAndEncode((char *)NULL, 50, 
					Source_Encoding);
	if (wf.str == NULL)
		return NULL;
