...
PySymtableEntry_New(struct symtable *st, char *name, int type, int lineno)
{
	...
	v = PyString_FromStringAndEncode(name, Source_Encoding);
	if (v == NULL)
		goto fail;
	ste->ste_name = v;
...
ste_repr(PySymtableEntryObject *ste)
{
	...
	sprintf(buf, "<符號表記項 %.100s(%ld), line %d>",
...
static struct memberlist ste_memberlist[] = {
	...
	{"代號",       T_OBJECT, OFF(ste_id), READONLY},
	{"name",     T_OBJECT, OFF(ste_name), READONLY},
	{"名稱",     T_OBJECT, OFF(ste_name), READONLY},
	{"symbols",  T_OBJECT, OFF(ste_symbols), READONLY},
	{"符號",  T_OBJECT, OFF(ste_symbols), READONLY},
	{"varnames", T_OBJECT, OFF(ste_varnames), READONLY},
	{"變數名", T_OBJECT, OFF(ste_varnames), READONLY},
	{"children", T_OBJECT, OFF(ste_children), READONLY},
	{"子項", T_OBJECT, OFF(ste_children), READONLY},
	{"type",     T_INT,    OFF(ste_type), READONLY},
	{"類型",     T_INT,    OFF(ste_type), READONLY},
	{"lineno",   T_INT,    OFF(ste_lineno), READONLY},
	{"行號",   T_INT,    OFF(ste_lineno), READONLY},
	{"optimized",T_INT,    OFF(ste_optimized), READONLY},
	{"最優化",T_INT,    OFF(ste_optimized), READONLY},
	{"nested",   T_INT,    OFF(ste_nested), READONLY},
	{"巢狀",   T_INT,    OFF(ste_nested), READONLY},
	{NULL}
};
