...
listmembers(struct memberlist *mlist)
{
	...
	if (v != NULL) {
		for (i = 0; i < n; i++)
			PyList_SetItem(v, i,
				       PyString_FromStringAndEncode(mlist[i].name,
					 Source_Encoding));
		if (PyErr_Occurred()) {
			Py_DECREF(v);
			v = NULL;
		...
		if (strcmp(l->name, name) == 0) {
			...
			case T_PSTRING:
				if (*(char**)addr == NULL) {
					...
					v = PyString_FromStringAndSizeAndEncode(
						(*(char**)addr)+1,
						**(unsigned char**)addr, 
						Source_Encoding);
				break;
			case T_PSTRING_INPLACE:
				v = PyString_FromStringAndSizeAndEncode(
					((char*)addr)+1,
					*(unsigned char*)addr, Source_Encoding);
				break;
#endif /* macintosh */
			case T_CHAR:
				v = PyString_FromStringAndSizeAndEncode(
					(char*)addr, 1, Source_Encoding);
				break;
			case T_OBJECT:
				v = *(PyObject **)addr;
				if (v == NULL)
