	       ...
	       PyObject *passthrough, int module_api_version)
{
	...
	if (doc != NULL) {
		v = PyString_FromStringAndEncode(doc, Source_Encoding);
		if (v == NULL || PyDict_SetItemString(d, "__doc__", v) != 0)
			return NULL;
		if (v == NULL || PyDict_SetItemString(d, "__說明__", v) != 0)
			return NULL;
		Py_DECREF(v);
	}
	return m;
...
do_mkvalue(char **p_format, va_list *p_va)
{
	for (;;) {
		...
		case 'c':
		{
			char p[1];
			p[0] = va_arg(*p_va, int);
			return PyString_FromStringAndSizeAndEncode(p, 1, 
					Source_Encoding);
		}
		...
		case 'z':
		{
			...
			if (str == NULL) {
				...
				v = PyString_FromStringAndSizeAndEncode(str, n, 
					Source_Encoding);
			}
			return v;
		}
...
{
	return PyModule_AddObject(m, name, PyString_FromStringAndEncode(value, 
					Source_Encoding));
}
