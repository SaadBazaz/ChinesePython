...
sys_displayhook(PyObject *self, PyObject *args)
{
	...
	if (outf == NULL) {
		...
		PyErr_SetString(PyExc_RuntimeError, "丟失了 sys.stdout");
		return NULL;
	}
	/* use Py_PRINT_RAW as last flag if want to use __str__ instead of __repr__ 
	   but __repr__ may be useful for encoding conversion, so be it.*/
...
sys_getdefaultencoding(PyObject *self, PyObject *args)
{
	if (!PyArg_ParseTuple(args, ":getdefaultencoding"))
		return NULL;
	return PyString_FromStringAndEncode(PyUnicode_GetDefaultEncoding(), 
			Source_Encoding);
}
...
static PyMethodDef sys_methods[] = {
	...
	{"顯示接口",	sys_displayhook, 1, displayhook_doc},
	{"exc_info",	sys_exc_info, 1, exc_info_doc},
	{"異常資訊",	sys_exc_info, 1, exc_info_doc},
	{"excepthook",	sys_excepthook, 1, excepthook_doc},
	{"異常接口",	sys_excepthook, 1, excepthook_doc},
	{"exit",	sys_exit, 0, exit_doc},
	{"退出",	sys_exit, 0, exit_doc},
	{"getdefaultencoding", sys_getdefaultencoding, 1,
	 getdefaultencoding_doc}, 
	{"讀目前編碼", sys_getdefaultencoding, 1,
	 getdefaultencoding_doc}, 
#ifdef COUNT_ALLOCS
	{"getcounts",	sys_getcounts, 1},
	{"讀記憶體調配次數",	sys_getcounts, 1},
#endif
#ifdef DYNAMIC_EXECUTION_PROFILE
	{"getdxp",	_Py_GetDXProfile, 1},
	{"讀動態效能檢查",	_Py_GetDXProfile, 1},
#endif
#ifdef Py_TRACE_REFS
	{"getobjects",	_Py_GetObjects, 1},
	{"讀實體",	_Py_GetObjects, 1},
	{"gettotalrefcount", sys_gettotalrefcount, 1},
	{"讀總參照", sys_gettotalrefcount, 1},
#endif
	{"getrefcount",	sys_getrefcount, 1, getrefcount_doc},
	{"讀參照",	sys_getrefcount, 1, getrefcount_doc},
	{"getrecursionlimit", sys_getrecursionlimit, 1,
	 getrecursionlimit_doc},
	{"讀遞歸上限", sys_getrecursionlimit, 1,
	 getrecursionlimit_doc},
	{"_getframe", sys_getframe, 1, getframe_doc},
	{"_讀執行框", sys_getframe, 1, getframe_doc},
#ifdef USE_MALLOPT
	{"mdebug",	sys_mdebug, 1},
#endif
	{"setdefaultencoding", sys_setdefaultencoding, 1,
	 setdefaultencoding_doc}, 
	{"設目前編碼", sys_setdefaultencoding, 1,
	 setdefaultencoding_doc}, 
	{"setcheckinterval",	sys_setcheckinterval, 1, setcheckinterval_doc}, 
	{"設同步間隔",	sys_setcheckinterval, 1, setcheckinterval_doc}, 
	{"setprofile",	sys_setprofile, 0, setprofile_doc},
	{"設效能檢查",	sys_setprofile, 0, setprofile_doc},
	{"setrecursionlimit", sys_setrecursionlimit, 1, setrecursionlimit_doc},
	{"設遞歸上限", sys_setrecursionlimit, 1, setrecursionlimit_doc},
	{"settrace",	sys_settrace, 0, settrace_doc},
	{"設追縱",	sys_settrace, 0, settrace_doc},
	{NULL,		NULL}		/* sentinel */
};
...
list_builtin_module_names(void)
{
	...
	for (i = 0; PyImport_Inittab[i].name != NULL; i++) {
		PyObject *name = PyString_FromStringAndEncode(
			PyImport_Inittab[i].name, Source_Encoding);
		if (name == NULL)
			break;
...
PySys_AddWarnOption(char *s)
{
	...
	str = PyString_FromStringAndEncode(s, Source_Encoding);
	if (str != NULL) {
...
_PySys_Init(void)
{
	...
	/* sysin = PyFile_FromFile(stdin, "<標準輸入>", "r", NULL);
	sysout = PyFile_FromFile(stdout, "<標準輸出>", "w", NULL);
	syserr = PyFile_FromFile(stderr, "<標準錯誤>", "w", NULL); */
	if (PyErr_Occurred())
		return NULL;
	PyDict_SetItemString(sysdict, "stdin", sysin);
	PyDict_SetItemString(sysdict, "標準輸入", sysin);
	PyDict_SetItemString(sysdict, "stdout", sysout);
	PyDict_SetItemString(sysdict, "標準輸出", sysout);
	PyDict_SetItemString(sysdict, "stderr", syserr);
	PyDict_SetItemString(sysdict, "標準錯誤輸出", syserr);
	/* Make backup copies for cleanup */
	PyDict_SetItemString(sysdict, "__stdin__", sysin);
	PyDict_SetItemString(sysdict, "__標準輸入__", sysin);
	PyDict_SetItemString(sysdict, "__stdout__", sysout);
	PyDict_SetItemString(sysdict, "__標準輸出__", sysout);
	PyDict_SetItemString(sysdict, "__stderr__", syserr);
	PyDict_SetItemString(sysdict, "__標準錯誤輸出__", syserr);
	PyDict_SetItemString(sysdict, "__displayhook__",
                             PyDict_GetItemString(sysdict, "displayhook"));
	PyDict_SetItemString(sysdict, "__顯示接口__",
                             PyDict_GetItemString(sysdict, "displayhook"));
	PyDict_SetItemString(sysdict, "__excepthook__",
                             PyDict_GetItemString(sysdict, "excepthook"));
	PyDict_SetItemString(sysdict, "__異常接口__",
                             PyDict_GetItemString(sysdict, "excepthook"));
	Py_XDECREF(sysin);
	Py_XDECREF(sysout);
	Py_XDECREF(syserr);
	PyDict_SetItemString(sysdict, "version",
			     v = PyString_FromStringAndEncode(Py_GetVersion(), 
					Source_Encoding));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "版本",
			     v = PyString_FromStringAndEncode(Py_GetVersion(), 
					Source_Encoding));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "中蟒版本",
			     v = PyString_FromStringAndEncode("0.4", 
					Source_Encoding));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "hexversion",
			     v = PyInt_FromLong(PY_VERSION_HEX));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "十六進版本",
			     v = PyInt_FromLong(PY_VERSION_HEX));
	Py_XDECREF(v);
	/*
	...
	PyDict_SetItemString(sysdict, "version_info",
			     v = Py_BuildValue("iiisi", PY_MAJOR_VERSION,
					       ...
					       PY_MICRO_VERSION, s,
					       PY_RELEASE_SERIAL));
	PyDict_SetItemString(sysdict, "版本訊息", v);
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "copyright",
			     v = PyString_FromStringAndEncode(Py_GetCopyright(), 
					Source_Encoding));
	PyDict_SetItemString(sysdict, "版權",
			     v = PyString_FromStringAndEncode(Py_GetCopyright(), 
					Source_Encoding));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "platform",
			     v = PyString_FromStringAndEncode(Py_GetPlatform(), 
					Source_Encoding));
	PyDict_SetItemString(sysdict, "操作平台",
			     v = PyString_FromStringAndEncode(Py_GetPlatform(), 
					Source_Encoding));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "executable",
			     v = PyString_FromStringAndEncode(
					Py_GetProgramFullPath(), Source_Encoding));
	PyDict_SetItemString(sysdict, "執行檔",
			     v = PyString_FromStringAndEncode(
					Py_GetProgramFullPath(), Source_Encoding));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "prefix",
			     v = PyString_FromStringAndEncode(Py_GetPrefix(), 
					Source_Encoding));
	PyDict_SetItemString(sysdict, "前綴",
			     v = PyString_FromStringAndEncode(Py_GetPrefix(), 
					Source_Encoding));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "exec_prefix",
		   v = PyString_FromStringAndEncode(Py_GetExecPrefix(), 
					Source_Encoding));
	PyDict_SetItemString(sysdict, "執行檔前綴",
		   v = PyString_FromStringAndEncode(Py_GetExecPrefix(), 
					Source_Encoding));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "maxint",
			     v = PyInt_FromLong(PyInt_GetMax()));
	PyDict_SetItemString(sysdict, "最大整數",
			     v = PyInt_FromLong(PyInt_GetMax()));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "builtin_module_names",
		   v = list_builtin_module_names());
	PyDict_SetItemString(sysdict, "內建模組名",
		   v = list_builtin_module_names());
	Py_XDECREF(v);
	{
		...
		PyDict_SetItemString(sysdict, "byteorder",
				     v = PyString_FromStringAndEncode(value, 
					Source_Encoding));
		PyDict_SetItemString(sysdict, "字元順位",
				     v = PyString_FromStringAndEncode(value_cp, 
					Source_Encoding));
		Py_XDECREF(v);
	}
#ifdef MS_COREDLL
	...
	PyDict_SetItemString(sysdict, "dll柄",
			     v = PyLong_FromVoidPtr(PyWin_DLLhModule));
	Py_XDECREF(v);
	PyDict_SetItemString(sysdict, "winver",
			     v = PyString_FromStringAndEncode(
					PyWin_DLLVersionString, Source_Encoding));
	PyDict_SetItemString(sysdict, "視窗版本",
			     v = PyString_FromStringAndEncode(
					PyWin_DLLVersionString, Source_Encoding));
	Py_XDECREF(v);
#endif
	PyDict_SetItemString(sysdict, "中文編碼",
		   v = PyString_FromStringAndEncode(Chinese_EncodingNames[Current_Encoding], Source_Encoding));
	Py_XDECREF(v);
	/* printf("chinese encoding set to :%i\n",Current_Encoding); */
	if (warnoptions == NULL) {
		warnoptions = PyList_New(0);
	}
	else {
	...
	if (warnoptions != NULL) {
		...
		PyDict_SetItemString(sysdict, "示警選項", warnoptions);
	}
...
makepathobject(char *path, int delim)
{
	...
	for (i = 0; ; i++) {
		...
		w = PyString_FromStringAndSizeAndEncode(path, (int) (p - path), 
					Source_Encoding);
		if (w == NULL) {
			Py_DECREF(v);
			return NULL;
...
PySys_SetPath(char *path)
{
	...
	if ((v = makepathobject(path, DELIM)) == NULL)
		...
		Py_FatalError("創建 sys.path 失敗");
	if (PySys_SetObject("path", v) != 0)
		/*Py_FatalError("can't assign sys.path");*/
		Py_FatalError("設定 sys.path 失敗");
	if (PySys_SetObject("路徑", v) != 0)
		Py_FatalError("設定 sys.路徑 失敗");
	Py_DECREF(v);
}
...
makeargvobject(int argc, char **argv)
{
	...
	if (av != NULL) {
		int i;
		for (i = 0; i < argc; i++) {
			PyObject *v = PyString_FromStringAndEncode(argv[i], 
					Source_Encoding);
			if (v == NULL) {
				Py_DECREF(av);
				av = NULL;
...
PySys_SetArgv(int argc, char **argv)
{
	...
	if (av == NULL)
		...
		Py_FatalError("不夠記憶體設定 sys.argv");
	if (PySys_SetObject("argv", av) != 0)
		/*Py_FatalError("can't assign sys.argv");*/
		Py_FatalError("設定 sys.argv 失敗");
	if (PySys_SetObject("參數", av) != 0)
		/*Py_FatalError("can't assign sys.argv");*/
		Py_FatalError("設定 sys.argv 失敗");
	if (path != NULL) {
		char *argv0 = argv[0];
...
#endif /* All others */
		a = PyString_FromStringAndSizeAndEncode(argv0, n, Source_Encoding);
		if (a == NULL)
			/*Py_FatalError("no mem for sys.path insertion");*/
			Py_FatalError("不夠記憶體在 sys.path 中插入新值");
		if (PyList_Insert(path, 0, a) < 0)
			/*Py_FatalError("sys.path.insert(0) failed");*/
			Py_FatalError("sys.path.insert(0) 失敗");
		Py_DECREF(a);
	}
	Py_DECREF(av);
}
...
mywrite(char *name, FILE *fp, const char *format, va_list va)
{
	...
	if (file == NULL || PyFile_AsFile(file) == fp)
		...
		if (vsprintf(buffer, format, va) >= sizeof(buffer))
		    ...
		    Py_FatalError("PySys_WriteStdout/err: 暫存溢出");
